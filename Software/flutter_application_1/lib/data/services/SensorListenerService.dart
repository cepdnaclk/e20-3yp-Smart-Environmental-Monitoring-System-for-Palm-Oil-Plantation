...
class SensorListenerService {
  ...
  final Set<String> _skippedDocs = {}; // ✅ Track which docs we’ve already skipped
  ...
  void startListening() {
    ...
    FirebaseFirestore.instance.collection('raw_readings').snapshots().listen((snapshot) {
      for (var doc in snapshot.docs) {
        ...
        // ✅ Skip first snapshot for each docId
        if (!_skippedDocs.contains(docId)) {
          _skippedDocs.add(docId);
          continue;
        }
        ...
        if (p != -1 && p < 3) _notify("Low Phosphorus", "Phosphorus=$p is detected at $now");
        if (k != -1 && k < 3) _notify("Low Potassium", "Potassium=$k is detected at $now");

        if (moisture != -1 && moisture > 75) {
          _notify("High Moisture", "$moisture% moisture detected at $now");
        }

        if (n != -1 && n > 120) _notify("High Nitrogen", "Nitrogen=$n is detected at $now");
        if (p != -1 && p > 120) _notify("High Phosphorus", "Phosphorus=$p is detected at $now");
        if (k != -1 && k > 120) _notify("High Potassium", "Potassium=$k is detected at $now");
      }
    });
  }
